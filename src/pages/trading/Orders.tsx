      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((order: any) => {
            ...
            return (
            <div key={order.id} className="rounded-xl p-3 bg-white dark:bg-[#141416] border border-gray-100 dark:border-white/[0.06] shadow-sm relative overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-medium text-gray-900 dark:text-white">{order.name}</span>
              ...
              <div className="text-[10px] text-gray-400 dark:text-gray-600 mt-2 pt-2 border-t border-gray-50 dark:border-white/[0.04]">
                <div>止损{order.stop_loss_pct}% | 回撤{order.trailing_pct}% | 激活{order.activation_pct}% | {order.max_hold_days}天</div>
                {order.reason && <div className="mt-0.5 text-gray-400">{order.reason}</div>}
                {order.status === 'executed' && order.executed_price && (
                  <div className="mt-0.5 text-green-500 dark:text-green-400">
                    成交 {order.executed_shares}股 @ {order.executed_price.toFixed(3)}
                  </div>
                )}
              </div>
